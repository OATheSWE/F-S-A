...
const PreviousMonth: React.FC = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (reportDocumentSnapshot.exists()) {
          ...
          const currentMonthYear = recordedData[`${previousMonth} ${currentYear}`];
  ...
  const handleWhatsAppSubmit = (e: any) => {
    ...
    let message;
    if (hasNonZeroInput) {
      message = `My Field Service Report For The Month Of ${previousMonth} - ${totalHours} Hours, ${totalVideos} Videos, ${totalPlacements} Placements, ${totalReturnVisits} Return Visits, ${totalBibleStudies} Bible Studies`;
    } else {
      message = `I Participated In The Feild Service For The Month Of ${previousMonth}.`;
    }
